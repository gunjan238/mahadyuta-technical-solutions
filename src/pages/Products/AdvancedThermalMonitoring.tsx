...
// import camBasic    from "@/assets/tcam.png";
// import camAdvanced from "@/assets/per_monitor.png";
// import camFixed    from "@/assets/thermalCam1.webp";
// import camAuto     from "@/assets/automation_cam.png";
...
//     photo: camBasic,
//     photoCovers: [
//       { top:"40%", left:"30%", width:"10%", height:"2%", bg:"rgba(40,30,20,0.82)", blur:true },
//     ],
...
//               <ThermalSystemSVG
//                 motorSrc={motorImg}
//                 cameraSrc={camera}          /* thermalCam1.webp — silver fixed camera */
...
const css = `
  ...
  /* ── Product card: narrower photo column (38%) + wider content (62%) ── */
  .ath-product-card {
    background:#fff;
    border:1px solid #eef0f4;
    border-radius:20px;
    ...
    box-shadow:0 4px 20px rgba(0,0,0,.06);
    display:grid;
    grid-template-columns:38% 62%;   /* CHANGED: was 1fr 1fr — photo column is now 38% */
    align-items:stretch;
    min-height:300px;                /* CHANGED: was 360px */
  }
  @media(max-width:860px){
    .ath-product-card {
      grid-template-columns:1fr;
  ...
  .ath-photo-panel {
    ...
    background:#f8fafc;
    border-right:1px solid #eef0f4;
    /* max-width keeps the image from stretching too wide on tall viewports */
    max-width:320px;
  }
  .ath-photo-panel img { transition:transform 0.5s ease; }
  .ath-product-card:hover .ath-photo-panel img { transform:scale(1.04); }
  @media(max-width:860px){
    .ath-photo-panel {
      ...
      border-right:none;
      border-bottom:1px solid #eef0f4;
      height:200px;           /* CHANGED: was 280px */
      max-width:100%;
    }
  }
  ...
  .ath-app-grid { display:grid; grid-template-columns:repeat(3,minmax(0,420px)); gap:20px; justify-content:center; }
  @media(max-width:960px){ .ath-app-grid{ grid-template-columns:repeat(2,minmax(0,420px)); } }
  @media(max-width:640px){ .ath-app-grid{ grid-template-columns:minmax(0,420px); } }

  .ath-int-grid { display:grid; grid-template-columns:repeat(4,minmax(0,300px)); gap:16px; justify-content:center; }
  @media(max-width:1100px){ .ath-int-grid{ grid-template-columns:repeat(2,minmax(0,360px)); } }
  @media(max-width:640px){ .ath-int-grid{ grid-template-columns:minmax(0,360px); } }

  .ath-why-grid { display:grid; grid-template-columns:repeat(4,minmax(0,320px)); gap:16px; justify-content:center; }
  @media(max-width:1100px){ .ath-why-grid{ grid-template-columns:repeat(2,minmax(0,360px)); } }
  @media(max-width:640px){ .ath-why-grid{ grid-template-columns:minmax(0,360px); } }
          ...
          <div>
            ...
            <div className="ath-svg-card">
              <ThermalSystemSVG
                motorSrc={motorImg}
                cameraSrc={camera}
                processingUnitSrc={dashboardImg}
              />
            </div>
          </div>
        </div>
